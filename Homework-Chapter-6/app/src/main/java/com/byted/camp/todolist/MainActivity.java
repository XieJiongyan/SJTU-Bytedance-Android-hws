...
//                        .setAction("Action", null).show();
        ...
        while(cursor.moveToNext()) {
            ...
            if(name.equals(TodoNote.LIST_NUM)) {
                Cursor cursor2 = database.query(TodoNote.LIST_NUM, null,
                        null, null, null, null, null);
                while (cursor2.moveToNext()) {
                    title_num = cursor2.getInt(cursor2.getColumnIndex(TodoNote.COLUMN_LIST));
                }
                cursor2.close();
                ContentValues values2 = new ContentValues();
                values2.put(TodoNote.COLUMN_LIST, title_num + 1);
                database.update(TodoNote.LIST_NUM, values2, null, null);
                break;
            }
        }
        long rowId = 0;
        if (title_num == 0) {
            String SQL_CREATE_NOTES =
                    "CREATE TABLE " + TodoNote.LIST_NUM
                            + "("
                            + TodoNote.COLUMN_LIST + " INTEGER)";
            database.execSQL(SQL_CREATE_NOTES);
            ContentValues values2 = new ContentValues();
            values2.put(TodoNote.COLUMN_LIST, 1);
            rowId = database.insert(TodoNote.LIST_NUM, null, values2);
        }
        cursor.close();
        String title = "todo" + title_num;
        setTitle(title);
